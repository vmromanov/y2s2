...
protected:
    ...
    fstream file_errors;
    string str;
    typedef void(Lex_Analyzator::* pfuncType)();//указатель на функицю состояния
    pfuncType pfunc;
    ...
    void End()
    {
        if (pfunc == &Lex_Analyzator::A1 || pfunc == &Lex_Analyzator::A2 || pfunc == &Lex_Analyzator::A3 || pfunc == &Lex_Analyzator::G1)
        {
            pfunc = &Lex_Analyzator::exit1;
        }
        else
            if (pfunc == &Lex_Analyzator::C1 || pfunc == &Lex_Analyzator::D1)
            {
                pfunc = &Lex_Analyzator::exit3;
            }
            else
                if (pfunc == &Lex_Analyzator::C2 || pfunc == &Lex_Analyzator::D2)
                {
                    pfunc = &Lex_Analyzator::exit4;
                }
                else
                    if (pfunc == &Lex_Analyzator::H1)
                    {
                        pfunc = &Lex_Analyzator::exit2;
                    }
                    else
                        if (pfunc == &Lex_Analyzator::D5)
                        {
                            pfunc = &Lex_Analyzator::exit5;
                        }
                        else
                            pfunc = &Lex_Analyzator::error2;
...
public:
    ...
    void InitializeTable() {
        ...
        for (int i = 0; i < 26; ++i) {
            ...
            table_alternatives[i].pfunc = &Lex_Analyzator::next;   // По умолчанию функция "next"
        }

        table_alternatives[0].letter = 'N' - 'A';
        table_alternatives[1].letter = 'D' - 'A';
        table_alternatives[1].pfunc = &Lex_Analyzator::A2q; // Функция для END 
        table_alternatives[2].letter = 'O' - 'A';
        table_alternatives[3].letter = 'R' - 'A';
        table_alternatives[3].pfunc = &Lex_Analyzator::F1b; // Функция для FOR 
        ...
        table_alternatives[6].pfunc = &Lex_Analyzator::E1a; //функция для GOTO
        table_alternatives[7].letter = 'S' - 'A';
        table_alternatives[8].letter = 'U' - 'A';
        table_alternatives[9].letter = 'B' - 'A';
        table_alternatives[9].pfunc = &Lex_Analyzator::E1b; // функция для GOSUB
        table_alternatives[10].letter = 'F' - 'A';
        table_alternatives[10].pfunc = &Lex_Analyzator::A2r; // Функция для IF 
        table_alternatives[11].letter = 'E' - 'A';
        table_alternatives[12].letter = 'T' - 'A';
        table_alternatives[12].pfunc = &Lex_Analyzator::F1a; // Функция для LET
        table_alternatives[13].letter = 'E' - 'A';
        table_alternatives[14].letter = 'X' - 'A';
        table_alternatives[15].letter = 'T' - 'A';
        table_alternatives[15].pfunc = &Lex_Analyzator::C1a; // Функция для NEXT
        ...
        table_alternatives[20].pfunc = &Lex_Analyzator::A2s; // Функция для RETURN
        table_alternatives[21].letter = 'M' - 'A';
        table_alternatives[21].pfunc = &Lex_Analyzator::G1a; // Функция для REM
        table_alternatives[22].letter = 'T' - 'A';
        table_alternatives[23].letter = 'E' - 'A';
        table_alternatives[24].letter = 'P' - 'A';
        table_alternatives[24].pfunc = &Lex_Analyzator::A2t; // Функция для STEP
        table_alternatives[25].letter = 'O' - 'A';
        table_alternatives[25].pfunc = &Lex_Analyzator::A2u; // Функция для TO
    }
    ...
    {
        RKL = NEXT;
        pfunc = &Lex_Analyzator::C1;
    }
    void C2a()
    {
        RKL = OPERAND;
        RI = RZN;
        pfunc = &Lex_Analyzator::C2;
    }
    void C2b()
    {
        AddLexeme();
    ...
    void D4a()
    {
        if (RZN == 1)
        {
            ...
            if (RZN == 2)
                RZ = -1;
        pfunc = &Lex_Analyzator::D4;
    }
    void D5a()
    {
        RZ = 1;
        RP = RZN;
        pfunc = &Lex_Analyzator::D5;
    }
    void D5b()
    {
        RP = RZN;
    ...
    {
        RI = RZN;
        pfunc = &Lex_Analyzator::F2;
    }
    void F3a()
    {
        RI += (RZN) * 26;
        pfunc = &Lex_Analyzator::F3;
    }
    void G1a()
    {
        RKL = REM;
